...
class Database:
    ...
    def create_database(self):
        try:
            ...
            self.c.execute('''CREATE TABLE IF NOT EXISTS students_lessons
                              (student_lesson_id SERIAL PRIMARY KEY,
                               student_id INTEGER,
                               lesson_id INTEGER)''')
            self.conn.commit()
    ...
    def add_student(self, student_name, student_identification):
        try:
            self.c.execute('''INSERT INTO students (student_name, student_identification)
                              VALUES (%s, %s)''', (student_name, student_identification))
            self.conn.commit()
    ...
    def add_lesson(self, lesson_date, lesson_course_id, teacher_id):
        try:
            self.c.execute('''INSERT INTO lessons (lesson_date, lesson_course_id, teacher_id)
                              VALUES (%s, %s, %s)''', (lesson_date, lesson_course_id, teacher_id))
            self.conn.commit()
    ...
    def add_student_lesson(self, student_id, lesson_id):
        try:
            self.c.execute('''INSERT INTO students_lessons (student_id, lesson_id)
                              VALUES (%s, %s)''', (student_id, lesson_id))
            self.conn.commit()
    ...
    def get_lessons_date(self, lesson_date):
        self.c.execute('''SELECT * FROM lessons WHERE lesson_date = %s''', (lesson_date,))
        return self.c.fetchall()
    ...
    def get_lesson_participants(self, lesson_id):
        self.c.execute('''SELECT sl.student_lesson_id, s.student_name, s.student_identification 
                          ...
                          WHERE sl.lesson_id = %s''', (lesson_id,))
        return self.c.fetchall()

    # Méthodes de suppression
    def remove_course(self, course_id):
        try:
            self.c.execute('''DELETE FROM courses WHERE course_id = %s''', (course_id,))
            self.conn.commit()
    ...
    def remove_student(self, student_id):
        try:
            self.c.execute('''DELETE FROM students WHERE student_id = %s''', (student_id,))
            self.conn.commit()
    ...
    def remove_lesson(self, lesson_id):
        try:
            self.c.execute('''DELETE FROM lessons WHERE lesson_id = %s''', (lesson_id,))
            self.conn.commit()
    ...
    def remove_student_lesson(self, student_lesson_id):
        try:
            self.c.execute('''DELETE FROM students_lessons WHERE student_lesson_id = %s''', (student_lesson_id,))
            self.conn.commit()
    ...
    def remove_lesson_and_links(self, lesson_id):
        try:
            ...
            self.c.execute('DELETE FROM students_lessons WHERE lesson_id = %s', (lesson_id,))
            self.c.execute('DELETE FROM lessons WHERE lesson_id = %s', (lesson_id,))
            self.conn.commit()
    ...
    def update_course(self, course_id, course_name, course_code, course_description, course_duration):
        try:
            self.c.execute('''UPDATE courses
                              SET course_name = %s, course_code = %s, course_description = %s, course_duration = %s
                              WHERE course_id = %s''',
    ...
    def update_student(self, student_id, student_name, student_identification):
        try:
            self.c.execute('''UPDATE students
                              SET student_name = %s, student_identification = %s
                              WHERE student_id = %s''',
    ...
    def update_lesson(self, lesson_id, lesson_date, lesson_course_id, teacher_id):
        try:
            self.c.execute('''UPDATE lessons
                              SET lesson_date = %s, lesson_course_id = %s, teacher_id = %s
                              WHERE lesson_id = %s''',
